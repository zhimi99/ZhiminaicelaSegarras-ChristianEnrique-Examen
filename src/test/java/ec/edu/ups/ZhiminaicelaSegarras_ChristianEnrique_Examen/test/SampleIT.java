package ec.edu.ups.ZhiminaicelaSegarras_ChristianEnrique_Examen.test;
...
public class SampleIT {
    ...
    public static Archive<?> getEarArchive() {
        ...
        File f = new File("./target/ZhiminaicelaSegarras-ChristianEnrique-Examen.war");
        if (f.exists() == false) {
            throw new RuntimeException("File " + f.getAbsolutePath() + " does not exist.");
        }
        WebArchive war = ShrinkWrap.create(ZipImporter.class, "ZhiminaicelaSegarras-ChristianEnrique-Examen.war").importFrom(f).as(WebArchive.class);
        
        // Add the package containing the test classes:
        war.addPackage("ec.edu.ups.ZhiminaicelaSegarras_ChristianEnrique_Examen.test");
